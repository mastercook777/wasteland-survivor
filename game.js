...
function freshMeta(){return{
 nextId:3,nextVid:8,
 backpack:[{id:1,type:'pistol',level:1,gx:1,gy:1,locked:true},{id:2,type:'ammo',level:1,gx:3,gy:1,locked:false}],
 vehiclePack:[{id:1,type:'engine',level:1,gx:0,gy:1,locked:true},{id:2,type:'turbo',level:1,gx:2,gy:1},{id:3,type:'mg',level:1,gx:3,gy:0,locked:true},{id:4,type:'belt',level:1,gx:5,gy:0},{id:5,type:'cannon',level:1,gx:3,gy:2,locked:true},{id:6,type:'loader',level:1,gx:5,gy:2},{id:7,type:'armor',level:1,gx:0,gy:3}],
...
let meta=freshMeta();
try{const s=localStorage.getItem('ws-portrait-v09');if(s){const o=JSON.parse(s);meta={...freshMeta(),...o,town:{...freshMeta().town,...o.town},vehicle:{...freshMeta().vehicle,...o.vehicle},pack:{...freshMeta().pack,...o.pack},vehicleGrid:{...freshMeta().vehicleGrid,...o.vehicleGrid}}}}catch(e){}
function save(){try{localStorage.setItem('ws-portrait-v09',JSON.stringify(meta))}catch(e){}}
function resetSave(){meta=freshMeta();save();state='route';ensureChoices(true)}
...
function buildStats(){
 ...
 for(const w of meta.backpack.filter(i=>ITEM[i.type].kind==='weapon')){
  const d=ITEM[w.type],lv=w.level||1;let interval=d.interval*Math.pow(.88,lv-1),damage=d.damage+(lv-1)*.45,range=d.range*(1+.08*(lv-1));
  for(const s of meta.backpack.filter(i=>ITEM[i.type].kind==='support'&&touching(i,w,ITEM))){const sl=s.level||1;if(s.type==='ammo')interval*=Math.max(.48,1-.16*sl);if(s.type==='scope'){range*=1+.16*sl;damage+=.2*sl}if(s.type==='apammo')damage+=.55*sl;}
  result.weapons.push({type:w.type,interval,damage,range,speed:d.speed,pellets:d.pellets||1,spread:d.spread||0,cd:rnd(0,.15)});
 }
 return result;
}
function vehicleStats(){
 let s={maxHull:8,maxFuel:24,speed:230,mgInterval:.22,cannonCd:2.9,rocketCount:0,rocketCd:3.5,armor:0};
 for(const it of meta.vehiclePack){const lv=it.level||1;if(it.type==='armor'){s.maxHull+=2*lv;s.armor+=lv}if(it.type==='fueltank')s.maxFuel+=8*lv;if(it.type==='rocket'){s.rocketCount++;s.rocketCd*=Math.max(.55,1-.08*(lv-1));}}
 const mg=meta.vehiclePack.find(i=>i.type==='mg'),cn=meta.vehiclePack.find(i=>i.type==='cannon'),en=meta.vehiclePack.find(i=>i.type==='engine');
 if(mg){for(const x of meta.vehiclePack.filter(i=>i.type==='belt'&&touching(i,mg,VEH)))s.mgInterval*=Math.max(.5,1-.15*(x.level||1));}
 if(cn){for(const x of meta.vehiclePack.filter(i=>i.type==='loader'&&touching(i,cn,VEH)))s.cannonCd*=Math.max(.5,1-.18*(x.level||1));}
 if(en){for(const x of meta.vehiclePack.filter(i=>i.type==='turbo'&&touching(i,en,VEH)))s.speed*=1+.10*(x.level||1);}
 return s;
}

function ensureChoices(force=false){if(meta.mapChoices.length&&!force)return;const types=shuffle(Object.keys(SITE));const townDue=meta.runStep>0&&meta.runStep%3===2;const arr=[];if(townDue)arr.push({id:'town-'+meta.roadLeg,type:'town',name:'RUSTWATER',short:'RUSTWATER',icon:'T',focus:'TRADE / REPAIR',danger:0,special:false});for(const t of types.slice(0,townDue?2:3)){const danger=1+Math.floor(Math.random()*3);arr.push({id:t+'-'+meta.roadLeg+'-'+Math.random(),type:'scavenge',site:t,name:SITE[t].name,short:SITE[t].short,icon:SITE[t].icon,focus:SITE[t].focus,danger,special:Math.random()<.17});}meta.mapChoices=shuffle(arr);}
function selectNode(i){ensureChoices();meta.selectedNode=meta.mapChoices[i]||meta.mapChoices[0];save();}
function fuelCost(){const n=meta.selectedNode;if(!n)return 0;if(n.type==='town')return 2;return 2+(n.danger||1)+(meta.route.fuel||0)}
...
function startRoad(){
 ...
 const vs=vehicleStats(),cost=fuelCost(),dry=meta.vehicle.fuel<cost,threat=roadThreat();
 meta.vehicle.fuel=Math.max(0,meta.vehicle.fuel-cost);
 roadGame={duration:dry?55:45,time:dry?55:45,elapsed:0,scroll:0,spawn:.6,mine:4.8,boss:false,dry,cost,threat,build:vs,player:{x:270,y:800,w:52,h:78,hp:meta.vehicle.hull==null?vs.maxHull:Math.min(meta.vehicle.hull,vs.maxHull),max:vs.maxHull,speed:vs.speed*(dry?.76:1),inv:0,mg:0,cannon:.9,rocket:1.3},enemies:[],bullets:[],enemyBullets:[],mines:[],drops:[],fx:[],kills:0,scrap:0,fuel:0,repairs:0};
 state='roadcombat';
 spawnRoadEnemy('bike');
 if(threat>=.92)spawnRoadEnemy('buggy');
}
function spawnRoadEnemy(force){
 ...
 const cfg=type==='bike'?{hp:2,spd:145,w:34,h:55,shoot:99,score:1}:type==='buggy'?{hp:4,spd:95,w:52,h:70,shoot:rnd(1.05,1.55),score:2}:type==='wartruck'?{hp:18,spd:48,w:82,h:110,shoot:.78,score:9}:{hp:9,spd:62,w:66,h:92,shoot:1.25,score:4};
 const shootScale=1.22/g.threat;
 roadGame.enemies.push({type,x:rnd(125,415),y:rnd(-160,-60),...cfg,maxHp:cfg.hp,shootCd:cfg.shoot*shootScale,sway:rnd(-1,1),ram:0,shootScale});
}
...
function fireRoad(kind,target){const g=roadGame,p=g.player,n=norm(target.x-p.x,target.y-p.y);const cfg=kind==='mg'?{spd:760,dmg:1,r:4}:kind==='cannon'?{spd:600,dmg:5,r:8}:{spd:510,dmg:3,r:7};g.bullets.push({x:p.x,y:p.y-38,vx:n.x*cfg.spd,vy:n.y*cfg.spd,life:1.5,damage:cfg.dmg,kind,r:cfg.r});if(kind==='cannon')g.fx.push({x:p.x,y:p.y-55,r:8,life:.18,color:C.yellow});}
...
function killRoad(i){
 const g=roadGame,e=g.enemies[i];g.kills+=e.score;g.fx.push({x:e.x,y:e.y,r:e.w*.7,life:.35,color:'#dd8040'});
 const missing=1-g.player.hp/g.player.max,repairChance=.08+missing*.16;
...
function updateRoad(dt){const g=roadGame,p=g.player;g.elapsed+=dt;g.time=Math.max(0,g.duration-g.elapsed);g.scroll+=340*dt;g.spawn-=dt;g.mine-=dt;p.inv=Math.max(0,p.inv-dt);p.mg-=dt;p.cannon-=dt;p.rocket-=dt;moveActor(p,dt,p.speed,105,435,610,850);
 const t=roadTarget(g,p,560);if(t&&p.mg<=0){fireRoad('mg',t);p.mg=g.build.mgInterval*(g.dry?1.35:1)}if(t&&p.cannon<=0){fireRoad('cannon',t);p.cannon=g.build.cannonCd*(g.dry?1.18:1)}if(t&&g.build.rocketCount&&p.rocket<=0){fireRoad('rocket',t);p.rocket=g.build.rocketCd}
 for(let i=g.bullets.length-1;i>=0;i--){const b=g.bullets[i];b.x+=b.vx*dt;b.y+=b.vy*dt;b.life-=dt;let hit=false;for(let j=g.enemies.length-1;j>=0;j--){const e=g.enemies[j];if(Math.abs(b.x-e.x)<e.w*.55&&Math.abs(b.y-e.y)<e.h*.5){e.hp-=b.damage;hit=true;g.fx.push({x:b.x,y:b.y,r:b.kind==='cannon'?22:8,life:.18,color:b.kind==='rocket'?'#d97642':C.yellow});if(e.hp<=0)killRoad(j);break}}if(hit||b.life<=0||b.y<-40)g.bullets.splice(i,1)}
 for(let i=g.enemies.length-1;i>=0;i--){const e=g.enemies[i];e.y+=e.spd*dt;e.x+=Math.sin(g.elapsed*1.7+e.sway)*18*dt;e.shootCd-=dt;e.ram-=dt;if(e.shootCd<=0&&e.y>80&&e.y<p.y-110&&e.type!=='bike'){const a=Math.atan2(p.y-e.y,p.x-e.x),shots=e.type==='wartruck'?3:1;for(let q=0;q<shots;q++){const aa=a+(q-(shots-1)/2)*.12;g.enemyBullets.push({x:e.x,y:e.y+e.h*.35,vx:Math.cos(aa)*360,vy:Math.sin(aa)*360,life:2.8})}e.shootCd=e.shoot*e.shootScale*(e.type==='wartruck'?.9:rnd(.9,1.15))}if(Math.abs(e.x-p.x)<(e.w+p.w)*.42&&Math.abs(e.y-p.y)<(e.h+p.h)*.42&&e.ram<=0){hurtRoad(e.type==='truck'||e.type==='wartruck'?2:1);e.hp-=2;e.ram=.8;if(e.hp<=0){killRoad(i);continue}}if(e.y>1030)g.enemies.splice(i,1)}
 ...
 if(g.mine<=0&&g.elapsed>7){g.mines.push({x:rnd(125,415),y:-25,r:16});if(g.threat>1.05&&g.elapsed/g.duration>.65&&Math.random()<Math.min(.4,(g.threat-1)*.8))g.mines.push({x:rnd(125,415),y:-70,r:16});g.mine=rnd(4.0,5.8)/g.threat}for(let i=g.mines.length-1;i>=0;i--){const m=g.mines[i];m.y+=210*dt;if(distance(m,p)<38){hurtRoad(2);g.fx.push({x:m.x,y:m.y,r:32,life:.3,color:'#db7c3f'});g.mines.splice(i,1);continue}if(m.y>990)g.mines.splice(i,1)}
 ...
 if(!g.boss&&g.elapsed/g.duration>.76){g.boss=true;spawnRoadEnemy('boss')}if(g.spawn<=0){spawnRoadEnemy();const extraChance=clamp((g.threat-.9)*.5,0,.30);if(g.elapsed/g.duration>.52&&Math.random()<extraChance)spawnRoadEnemy();g.spawn=rnd(.95,1.35)/(meta.route.spawn||1)/g.threat}
 if(p.hp<=0){meta.arrived=false;state='roadfail';save();return}if(g.time<=0){meta.arrived=true;meta.vehicle.hull=Math.max(1,p.hp);const bf=meta.route.id==='salt'?2:0,bs=meta.route.id==='raider'?2:0;meta.vehicle.scrap+=g.scrap+bs;meta.vehicle.fuel=Math.min(g.build.maxFuel,meta.vehicle.fuel+g.fuel+bf);g.scrap+=bs;g.fuel+=bf;const pool=['belt','loader','turbo','armor','fueltank','rocket'];const t=choice(pool),lv=meta.selectedNode?.danger===3&&Math.random()<.25?2:1;meta.pendingVehicle.push(makeVehicle(t,lv));g.reward=t;g.rewardLv=lv;if(meta.route.id==='raider'&&Math.random()<.55)meta.pendingVehicle.push(makeVehicle(choice(pool),1));state='roadresult';save()}}
...
function startScavenge(){
 ...
 game={site,danger,special,time:90,elapsed:0,worldH,cameraY:0,player:{x:270,y:1240,r:15,hp:b.maxHp,max:b.maxHp,speed:b.speed,inv:0,med:b.medCharges},build:b,obstacles,crates:[],loot:[],enemies:[],bullets:[],enemyBullets:[],grenades:[],fx:[],haul:[],weaponCd:b.weapons.map(w=>({...w,cd:rnd(0,.25)})),exit:{x:270,y:1330,progress:0},search:null,spawn:.2};
 const spots=[[100,120],[270,135],[440,120],[105,300],[430,310],[90,690],[450,700],[270,530],[270,260]];
 shuffle(spots).slice(0,5+danger).forEach((p,i)=>game.crates.push({x:p[0]+rnd(-12,12),y:Math.round(p[1]*scaleY+yOffset+rnd(-12,12)),opened:false,rare:special&&i===0,progress:0}));
 for(let i=0;i<4+danger*2;i++)spawnScavEnemy();
 if(special)game.enemies.push(makeScavEnemy('elite',270,300));
 game.cameraY=clamp(game.player.y-620,0,game.worldH-H);
 state='play';
}
function makeScavEnemy(type,x=rnd(60,480),y=rnd(250,game?.worldH?game.worldH-230:1200)){if(!type)type=choice(SITE[game.site].enemy);const e={type,x,y,cd:rnd(.4,1.2),inv:0};if(type==='melee')Object.assign(e,{hp:3,spd:74,r:15,color:'#78945f'});if(type==='gunner')Object.assign(e,{hp:4,spd:48,r:15,color:'#9b6d45'});if(type==='grenadier')Object.assign(e,{hp:5,spd:42,r:17,color:'#8d5d48'});if(type==='elite')Object.assign(e,{hp:12,spd:58,r:21,color:'#b34f3d'});e.max=e.hp;return e}
function spawnScavEnemy(type){if(game.enemies.length>18)return;let e=makeScavEnemy(type);let tries=0;while(distance(e,game.player)<230&&tries++<20)e=makeScavEnemy(type);game.enemies.push(e)}
...
function openCrate(c){c.opened=true;const n=c.rare?3:1+Math.floor(Math.random()*2);for(let i=0;i<n;i++){const t=chooseLoot();meta.cargo[t]++;game.loot.push({x:c.x+rnd(-18,18),y:c.y+rnd(-18,18),type:t,life:1.2})}const g=gearDrop(c.rare);if(g)game.haul.push(g);if(game.site==='junkyard'&&Math.random()<(c.rare?.38:.12))meta.pendingVehicle.push(makeVehicle(choice(['belt','loader','turbo','armor','fueltank','rocket']),c.rare&&Math.random()<.25?2:1));notice=c.rare?'RARE CACHE OPENED':'SUPPLIES SECURED';noticeT=.8;save()}
function fireScav(w,target){const p=game.player,base=Math.atan2(target.y-p.y,target.x-p.x);for(let i=0;i<w.pellets;i++){const a=base+(i-(w.pellets-1)/2)*(w.spread||0);game.bullets.push({x:p.x,y:p.y,vx:Math.cos(a)*w.speed,vy:Math.sin(a)*w.speed,life:w.range/w.speed,damage:w.damage,r:w.pellets>1?3:4})}}
function updateScavenge(dt){const g=game,p=g.player;g.elapsed+=dt;g.time=Math.max(0,90-g.elapsed);p.inv=Math.max(0,p.inv-dt);let mx=0,my=0;if(keys.has('a')||keys.has('arrowleft'))mx--;if(keys.has('d')||keys.has('arrowright'))mx++;if(keys.has('w')||keys.has('arrowup'))my--;if(keys.has('s')||keys.has('arrowdown'))my++;mx+=joy.dx;my+=joy.dy;if(Math.hypot(mx,my)>.08){const n=norm(mx,my);moveWithObstacles(p,n.x*p.speed*dt,n.y*p.speed*dt,p.r,g.obstacles,[28,512,170,g.worldH-90])}const cameraTarget=clamp(p.y-620,0,g.worldH-H);g.cameraY+=(cameraTarget-g.cameraY)*Math.min(1,dt*7);
 ...
 for(let i=g.bullets.length-1;i>=0;i--){const b=g.bullets[i];b.x+=b.vx*dt;b.y+=b.vy*dt;b.life-=dt;let hit=false;for(let j=g.enemies.length-1;j>=0;j--){const e=g.enemies[j];if(distance(b,e)<e.r+b.r){e.hp-=b.damage;hit=true;g.fx.push({x:b.x,y:b.y,r:7,life:.15,color:C.yellow});if(e.hp<=0){if(Math.random()<.08+g.build.luck){const gear=gearDrop(false);if(gear)g.haul.push(gear)}g.enemies.splice(j,1)}break}}if(hit||b.life<=0)g.bullets.splice(i,1)}
 for(let i=g.enemies.length-1;i>=0;i--){const e=g.enemies[i],d=distance(e,p);e.cd-=dt;if(e.type==='melee'||e.type==='elite'){const n=norm(p.x-e.x,p.y-e.y);e.x+=n.x*e.spd*dt;e.y+=n.y*e.spd*dt;if(d<e.r+p.r+4&&e.cd<=0){hurtPlayer(e.type==='elite'?2:1);e.cd=.9}}else{const desired=e.type==='gunner'?235:275;if(d>desired+35){const n=norm(p.x-e.x,p.y-e.y);e.x+=n.x*e.spd*dt;e.y+=n.y*e.spd*dt}else if(d<desired-45){const n=norm(e.x-p.x,e.y-p.y);e.x+=n.x*e.spd*dt;e.y+=n.y*e.spd*dt}if(e.cd<=0){if(e.type==='gunner'){const n=norm(p.x-e.x,p.y-e.y);g.enemyBullets.push({x:e.x,y:e.y,vx:n.x*280,vy:n.y*280,life:2.1})}else{g.grenades.push({x:p.x+rnd(-35,35),y:p.y+rnd(-35,35),t:1.15,r:54})}e.cd=e.type==='gunner'?rnd(1.25,1.75):rnd(2.2,3.0)}}}
 for(let i=g.enemyBullets.length-1;i>=0;i--){const b=g.enemyBullets[i];b.x+=b.vx*dt;b.y+=b.vy*dt;b.life-=dt;if(distance(b,p)<p.r+5){hurtPlayer(1);g.enemyBullets.splice(i,1);continue}if(b.life<=0)g.enemyBullets.splice(i,1)}for(let i=g.grenades.length-1;i>=0;i--){const gr=g.grenades[i];gr.t-=dt;if(gr.t<=0){if(distance(gr,p)<gr.r)hurtPlayer(2);g.fx.push({x:gr.x,y:gr.y,r:gr.r,life:.35,color:'#dd6d43'});g.grenades.splice(i,1)}}
 ...
 g.spawn-=dt;const pressure=g.elapsed>45?1.45:1;if(g.spawn<=0){spawnScavEnemy();if(g.elapsed>65&&Math.random()<.45)spawnScavEnemy();g.spawn=rnd(2.2,3.5)/pressure/(g.danger===3?1.3:1)}for(let i=g.fx.length-1;i>=0;i--){g.fx[i].life-=dt;if(g.fx[i].life<=0)g.fx.splice(i,1)}if(p.hp<=0){meta.pendingHaul=[];state='dead';save()}}
...
function startPack(returnState='route',advance=false,haul=[]){meta.pendingHaul=haul.length?haul:meta.pendingHaul;packReturn=returnState;packAdvance=advance;packDrag=null;state='pack'}
...
addEventListener('keydown',e=>{const k=e.key.toLowerCase();keys.add(k);if(['arrowup','arrowdown','arrowleft','arrowright',' '].includes(k))e.preventDefault();if(state==='menu'&&(k==='enter'||k===' ')){state='route';ensureChoices()}else if(state==='route'&&(k==='enter'||k===' ')){if(!meta.selectedNode)selectNode(0);startRoad()}else if(state==='roadresult'&&(k==='enter'||k===' ')){meta.selectedNode?.type==='town'?enterTown():startScavenge()}else if((state==='roadfail'||state==='dead')&&(k==='enter'||k===' ')){state='route';meta.selectedNode=null;ensureChoices(true)}else if(state==='route'&&(k==='b'||k==='i'))startPack('route',false,[]) ;else if(state==='route'&&k==='v')startVehicleBay('route')});
addEventListener('keyup',e=>keys.delete(e.key.toLowerCase()));
canvas.addEventListener('pointerdown',e=>{const p=canvasPoint(e);if(state==='roadcombat'||state==='play'){if(p.y>500)joyStart(e,p);return}if(state==='menu'){state='route';ensureChoices();return}if(state==='route'){routeTap(p);return}if(state==='roadresult'){meta.selectedNode?.type==='town'?enterTown():startScavenge();return}if(state==='roadfail'||state==='dead'){state='route';meta.selectedNode=null;ensureChoices(true);return}if(state==='pack'){packDown(p);return}if(state==='vehicle'){vehicleDown(p);return}if(state==='town'){townTap(p);return}});
...
function routeTap(p){ensureChoices();const pos=routeNodePositions();for(let i=0;i<pos.length;i++){const a=pos[i];if(Math.hypot(p.x-a.x,p.y-a.y)<60){selectNode(i);return}}if(p.y>835&&p.x>55&&p.x<485){if(!meta.selectedNode)selectNode(0);startRoad();return}if(p.y<105&&p.x<165)startPack('route',false,[]);else if(p.y<105&&p.x>375)startVehicleBay('route')}
...
function drawEnemyVehicle(e){const col=e.type==='wartruck'?'#6d594c':e.type==='truck'?'#75624c':e.type==='bike'?'#9a7049':'#806448';drawVehicle(e.x,e.y,e.type==='wartruck'?1.2:e.type==='bike'?.65:.85,col);if(e.maxHp>4)bar(e.x-e.w*.45,e.y-e.h*.62,e.w*.9,5,e.hp/e.maxHp,C.red)}
...
function drawScavenge(){
 ...
 for(const gr of g.grenades){ctx.save();ctx.globalAlpha=.28+.35*Math.sin(performance.now()/70);ctx.fillStyle=C.red;ctx.beginPath();ctx.arc(gr.x,gr.y,gr.r,0,6.28);ctx.fill();ctx.restore();ctx.strokeStyle=C.red;ctx.lineWidth=3;ctx.beginPath();ctx.arc(gr.x,gr.y,gr.r,0,6.28);ctx.stroke()}
 ...
 if(g.special)text('RARE SIGNAL',270,148,12,C.yellow,'center',900);
 drawJoy();drawNotice();
}
...
function drawScavEnemy(e){ctx.fillStyle=e.color;ctx.beginPath();ctx.arc(e.x,e.y,e.r,0,6.28);ctx.fill();text(e.type==='gunner'?'G':e.type==='grenadier'?'!':e.type==='elite'?'E':'',e.x,e.y+4,10,'#171912','center',900);if(e.type!=='melee')bar(e.x-18,e.y-e.r-11,36,4,e.hp/e.max,C.red)}
